/* =====================================================================
   ...
   환경변수
     ...
     GEMINI_MODEL    (선택)  지정하면 그 모델만 씀. 비우면 아래 후보를 차례로 시도
   ===================================================================== */

/* 후보 모델 (앞에서부터 시도한다)
   - 구글이 모델을 자주 갈아치우기 때문에, 하나가 막히면 다음 것으로 자동 재시도한다.
   - 2026-09-15 실측: gemini-2.0-flash / gemini-2.5-flash 는 신규 사용자에게 404.
     구글이 대체 모델로 gemini-3.6-flash 를 안내함.
   - GEMINI_MODEL 환경변수를 지정하면 그 모델만 쓴다. */
const FALLBACK_MODELS = ['gemini-3.6-flash', 'gemini-flash-latest', 'gemini-3.1-flash-lite'];
const RETRY_STATUS = [404, 429, 500, 502, 503, 504];   // 다음 모델로 넘어갈 상태코드
...
module.exports = async (req, res) => {
  ...
  for (const model of models) {
    ...
    try {
      ...
      if (!r.ok) {
        ...
        tried.push({ model, status: r.status, error: String(raw).replace(key, '***') });
        if (RETRY_STATUS.includes(r.status) && model !== models[models.length - 1]) continue;  // 다음 모델로
        return res.status(r.status).json({
          ...
          hint: r.status === 400 || r.status === 403
            ? 'API 키가 잘못됐거나, 키에 이 모델 사용 권한이 없습니다.'
            : r.status === 404
              ? '후보 모델이 모두 막혔습니다. GEMINI_MODEL 환경변수에 현재 쓸 수 있는 모델명을 직접 지정하세요.'
              : r.status === 429
                ? '호출 한도를 초과했습니다. 잠시 뒤 다시 시도하세요.'
                : r.status === 503
                  ? '구글 쪽이 일시적으로 혼잡합니다. 잠시 뒤 다시 시도하세요.'
                  : undefined,
        });
      }
      ...
      if (!text) {
        tried.push({ model, status: 200, error: '본문이 빈 응답' });
        if (model !== models[models.length - 1]) continue;
        return res.status(502).json({
          ok: false, error: '응답은 받았지만 본문이 비어 있습니다.', tried,
      ...
      return res.status(200).json({
        ...
        text,
        model,                                   // 실제로 답한 모델
        fellBack: tried.length > 0,              // 앞 후보가 실패해 넘어왔는지
        tried,
        ms: Date.now() - started,
      ...
      tried.push({ model, error: aborted ? '시간 초과' : String(e && e.message || e).replace(key, '***') });
      if (model !== models[models.length - 1]) continue;
  ...
  return res.status(500).json({ ok: false, error: '시도할 모델이 없습니다.', tried });
};
