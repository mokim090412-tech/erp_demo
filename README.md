...
| `GEMINI_MODEL` | (선택) 비워두는 것을 권장 — 아래 "모델 자동 재시도" 참고 |
...
구글이 모델을 자주 갈아치웁니다. 실제로 **2026-09-15 기준 `gemini-2.0-flash`와
`gemini-2.5-flash`는 신규 사용자에게 404로 막혀 있습니다.**
그래서 서버 함수가 아래 순서로 자동 재시도하도록 만들었습니다.

```
gemini-3.6-flash  →  gemini-flash-latest  →  gemini-3.1-flash-lite
```

404(모델 없음)·429(한도)·503(혼잡)이 나면 다음 후보로 넘어갑니다.
성공하면 화면에 **실제로 답한 모델 이름**이 표시됩니다.

> `GEMINI_MODEL` 환경변수를 지정하면 재시도 없이 그 모델만 씁니다.
> 특별한 이유가 없으면 **비워두세요.**
...
| 후보 모델이 모두 막혔습니다 (404) | 구글이 모델을 또 바꾼 경우. `GEMINI_MODEL`에 현재 쓸 수 있는 모델명을 직접 지정 |
| 구글 쪽이 일시적으로 혼잡 (503) | 잠시 뒤 재시도. 자동으로 다음 후보 모델도 시도함 |
